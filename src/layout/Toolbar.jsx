...
function Toolbar({setShowToolBar}) {
  ...
  const onNodeClick = (event, label, nodeType = 'customNode') => {
    debugger;
    event.dataTransfer.setData('type', nodeType);
    event.dataTransfer.setData('label', label);
  ...
  return (
    <>
      ...
      -translate-x-full sm:translate-x-0 border-r border-gray-200 background bg-gray-200" aria-label="Sidebar">
        <div className="h-full px-3 py-4 overflow-y-auto">
          <ul className="space-y-2 font-medium">
            <li>
              <button className="flex items-center p-1 py-2 text-gray-900 bg-white rounded-lg hover:shadow-md 
              hover:border-gray-200 border border-white my-3 w-full" onClick={(event) => onNodeClick(event, 'Message')} onDragStart={(event) => onDragStart(event, 'Message')} draggable>
